...
function populateTransactionsTable(
  filteredTransactions: Transaction[] = transactions
) {
...
function exportToCSV() {
  ...
  const csvContent = [
    headers.join(","),
    ...transactions.map((t) =>
      [t.id, `"${t.description}"`, t.amount, t.date, t.status].join(",")
    ),
  ].join("\n");
      ...
      Object.values(Chart.instances).forEach((chart: any) => {
        if (chart.config?.options) {
          ...
          if (chart.config.options.scales) {
            Object.values(chart.config.options.scales).forEach((scale: any) => {
